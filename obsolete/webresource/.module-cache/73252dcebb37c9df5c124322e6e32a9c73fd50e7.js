	...
	return React.createClass({
		...
		render:function(){
			return React.createElement("div", React.__spread({className: "select"},  this.state.events), 
						React.createElement("ul", {style: {marginTop:this.state.marginTop+"px",transitionDuration:this.state.transitionDuration,transform:this.state.translateY}}, 
							this.createItems()
						), 
		...
		ontouchend:function(e){
			...
			if(marginTop != targetMarginTop){
				state.marginTop = 0;
				state.translateY = "translateY("+targetMarginTop+"px)";
			}
