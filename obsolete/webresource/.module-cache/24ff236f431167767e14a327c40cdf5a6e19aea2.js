		...
		create:function(props){
			switch(props.icon){
				...
				case 'option-vertical':
					return React.createElement(UIToolbars.moremenu, React.__spread({},  props));
			}
		},
		createMenuList:function(props){
			var children = [],
				moremenu = this.create('moremenu');
			_.each(props,$.proxy(function(item){
				item = this.create(item);
				item && children.push(item);
			},this));
			return React.createElement(UIToolbars.moreMenuContainer, {moremenu: moremenu}, children);
		}
	});
});