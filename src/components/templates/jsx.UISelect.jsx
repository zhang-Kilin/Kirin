define('jsx.UISelect',['react'],function(React){
	 ...
	 * ]
	 */
	var UISelect = React.createClass({
		...
		render:function(){
			return <div className="select" {...this.state.events}>
						<ul style={{marginTop:this.state.marginTop+"px"}}>
							{this.createItems()}
						</ul>
						<div className="mask">
						</div>
		...
		createItems:function(){
			...
			if(this.state.children){
				_.each(this.state.children, $.proxy(function(item){
					results.push(<li className={item.selected?"current":""}>{item.text}</li>);
				},this));
			}
			return results;
		},
		...
		ontouchend:function(e){
			...
			if(changed){
				...
				setTimeout($.proxy(function(){
					item = _.extend({},this.props.children[index]);
					_.isFunction(this.props.onchange) && this.props.onchange(item,this.state.children,this.props);	
				},this),0);
			}
	...
	UISelect.TRANSACTION_DURATION = 150;
